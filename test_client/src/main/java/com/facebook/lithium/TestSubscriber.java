...
public class TestSubscriber<T> implements Subscriber<T>, Subscription, Disposable {
    ...
    /**
     * A function that utilizes a countdownlatch's timeout to count the number of onNext calls this subscriber has
     * experienced.
     * @param n
     * @param time
     * @param unit
     ...
     */
    public final boolean awaitAtMost(long n, long time, TimeUnit unit) throws InterruptedException {
        numOnNext.await(time, unit);
        long numValues = Int.MaxValue() - numOnNext.getCount();
        if (numValues <= n) pass("got " + numValues + " out of " + n + " values", true);
        else fail("got more values than expected");
